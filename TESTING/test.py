...
#     all_job_ids.update(recommended_jobs)
#
#     # Get ground truth job preferences for the candidate
...
#     all_job_ids.update(ground_truth_job_ids)
#
#     # Consider only the first 5 jobs from the ground truth
#     top_5_ground_truth_job_ids = ground_truth_job_ids[:5]  # Assuming top 5
#
#     # Debugging outputs
#     print(f"Candidate ID: {candidate_id}")
#     print(f"Recommended Jobs: {recommended_jobs}")
#     print(f"Top 5 Ground Truth Jobs: {top_5_ground_truth_job_ids}")
#
#     # Evaluate each job against ground truth and recommendations
#     for job in all_job_ids:
#         true_label = 0
#         predicted_label = 0
#
#         if job in top_5_ground_truth_job_ids:
#             true_label = 1
#
...
# [[787   0]
#  [106   3]]
# True Negative (TN): 787, False Positive (FP): 0, False Negative (FN): 106, True Positive (TP): 3
# Accuracy: 0.88
# Precision: 1.00
# Recall: 0.03
# F1 Score: 0.05
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Load the output.csv file
# df = pd.read_csv('output.csv', dtype={'jobId': str})
#
# # Initialize lists to store the true labels and predicted labels
# true_labels = []
# predicted_labels = []
...
#     top_5_ground_truth_jobs = ground_truth_jobs[:1]  # Assuming top 5
#
#     # Debugging outputs
#     print(f"Candidate ID: {candidate_id}")
#     print(f"Recommended Job: {recommended_job_id}")
#     print(f"Top 5 Ground Truth Jobs: {top_5_ground_truth_jobs}")
#
#     # Check if recommended job is in the top 5 ground truth jobs
#     if recommended_job_id in top_5_ground_truth_jobs:
...
#
#     # For each job in the top 5 ground truth, check if it was not recommended
#     for job in top_5_ground_truth_jobs:
#         if job != recommended_job_id:
#             true_labels.append(1)  # Relevant job
#             predicted_labels.append(0)  # Not recommended
#
#     # Calculate true negatives (jobs not in top 5 ground truth and not recommended)
#     for job in all_job_ids:
#         if job not in top_5_ground_truth_jobs and job != recommended_job_id:
